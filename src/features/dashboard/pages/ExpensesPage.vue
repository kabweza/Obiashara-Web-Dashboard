  ...
  <div class="expenses-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">{{ displayed.length }} {{ t('records') }} · TZS {{ formatCurrency(totalExpenses) }}</p>
      </div>
    ...
    <!-- Tabs -->
    <div class="tabs">
      <button class="tab" :class="{ active: tab === 'active' }" @click="tab = 'active'; load()">{{ t('active') }}</button>
...
.category-cards { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 12px; }
.cat-card { background: white; border-radius: 12px; padding: 14px 16px; border: 1px solid #f3f4f6; }
.cat-name { display: block; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.4px; margin-bottom: 4px; }
.cat-amount { display: block; font-size: 15px; font-weight: 800; color: #111827; margin-bottom: 8px; }
.cat-bar { height: 4px; background: #f3f4f6; border-radius: 2px; overflow: hidden; }
.cat-fill { height: 100%; border-radius: 2px; transition: width 0.5s ease; }
...
.data-table td { padding: 13px 16px; border-bottom: 1px solid #f9fafb; vertical-align: middle; color: #374151; }
...
.cat-badge { background: #f3f4f6; color: #374151; padding: 2px 8px; border-radius: 6px; font-size: 11px; font-weight: 600; }
